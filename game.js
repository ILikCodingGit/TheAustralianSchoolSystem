...
function formatText(template, a, b) {
  ...
  if (b) result = result.replace(/\{B\}/g, `<span class="nameTag">${b.name}</span>`);
  return result;
}
...
function getBestWeapon(participant) {
  ...
  for (const wId of weaponIds) {
    if (participant.inventory.includes(wId)) return eventsData.shopItems.find(i => i.id === wId);
  }
  return null;
}
...
function getDefense(participant) {
  ...
  for (const iId of participant.inventory) {
    ...
    if (item && item.defense) def += item.defense;
  }
  return def;
}
...
function resolveCombat(attacker, defender) {
  ...
  if (weapon) {
    ...
    combatMsg = combatEntry ? formatText(combatEntry.text, attacker, defender) : formatText(randFrom(eventsData.baseCombatMessages), attacker, defender);
  ...
  for (let i = 0; i < eventCount && shuffled.filter(p => p.alive).length > 1; i++) {
    ...
    if (roll < 0.30 && b) {
      ...
      if (followUps.length) {
        ...
        if (ev.moneyGain) { a.money += ev.moneyGain; text += ` (+$${ev.moneyGain})`; }
        if (ev.healthGain) { a.health = Math.min(100, a.health + ev.healthGain); text += ` (+${ev.healthGain} HP)`; }
      ...
      let text = formatText(ev.text, a, b);
      if (ev.moneyGain) { a.money += ev.moneyGain; text += ` (+$${ev.moneyGain})`; }
      if (ev.moneyLoss && b) { b.money = Math.max(0, b.money - ev.moneyLoss); text += ` (${b.name} -$${ev.moneyLoss})`; }
...
function processNormalEvent(ev, a, b, dayLog) {
  let text = formatText(ev.text, a, b);
  if (ev.setFlag) a.flags[ev.setFlag] = true;
  dayLog.push({ type: 'normal', text });
}

function checkWinner() {
  const alive = gameState.participants.filter(p => p.alive);
  return alive.length === 1 ? alive[0] : alive.length === 0 ? gameState.participants[gameState.participants.length - 1] : null;
}
...
function nextDay() {
  ...
  const w = checkWinner();
  if (w) setTimeout(() => showWinner(w), 400);
}
...
function renderLog() {
  ...
  gameState.eventLog.forEach(dayEntry => {
    ...
    dayEntry.events.forEach(ev => {
      ...
      const tagClass = ev.type === 'death' ? 'deathTag' : ev.type.startsWith('combat') ? 'combatTag' : ev.type === 'followup' ? 'followTag' : 'eventTag';
...
function showWinner(winner) {
  ...
  document.getElementById('honorRoll').innerHTML = `
    <div class="honorItem"><span class="honorLabel">⚔️ Most Dangerous</span><span class="honorVal">${mostKills.name} (${mostKills.kills} kills)</span></div>
